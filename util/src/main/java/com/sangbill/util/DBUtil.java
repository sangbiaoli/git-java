...
public class DBUtil {
	static private DBUtil instance; // The single instance
	 ...
	 */
	static synchronized public DBUtil getInstance() {
		if (instance == null) {
			instance = new DBUtil();
		}
		clients++;
	...
	private void createPools() {
		String url = "jdbc:mysql://localhost:3306/ciyun_rpt";		
		String user = "root";
		String password = "123456";
		int max  =50;
		
		pool = new DBConnectionPool(url,user, password, max);
		log("Initialized pool ");
	...
	private void init() {
		try {
			log = new PrintWriter(new FileWriter("db.log", true), true);
		} catch (IOException e) {
			e.printStackTrace();
		}
		loadDrivers();
	...
	class DBConnectionPool {
		...
		public synchronized Connection getConnection() {
			...
			if (freeConnections.size() > 0) {
				...
				try {
					if (con.isClosed()) {
						log("Removed bad connection from ");
		...
		public synchronized void release() {
			...
			while (allConnections.hasMoreElements()) {
				...
				try {
					...
					log(e, "Can't close connection for pool ");
				}
			}
